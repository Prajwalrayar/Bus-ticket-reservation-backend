...
public interface LocationRepository extends JpaRepository<Location, Long> {
    ...
    @Query("SELECT l FROM Location l WHERE l.isActive = true AND (LOWER(l.name) = LOWER(:query) OR l.locationId IN (SELECT la.location.locationId FROM LocationAlias la WHERE la.isActive = true AND LOWER(la.alias) = LOWER(:query)))")
    java.util.Optional<Location> findByExactNameOrAlias(@Param("query") String query);
